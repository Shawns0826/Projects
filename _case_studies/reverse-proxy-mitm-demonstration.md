...
Nothing else about the client needs to change—in fact it is more convenient for the attacker to leave the client as is. Now they would map every endpoint the client requests, and ensure they intercept any that involve authorization or authentication. The attacker would set up their own custom logic for dealing with authorization or authentication—validating usernames and passwords, assigning session tokens, registering accounts, and so on. That control plane is non-trivial; in [Part 3: RBAC Admin Panel]({{ '/case-studies/rbac-admin-panel/' | relative_url }}) we walk through a reference implementation of subtree-scoped roles, credit delegation, and audit logging—the same kind of infrastructure an attacker’s broker would need to run their service. Anything else, such as fetching media, is upstreamed to the victim server and then returned to the user. This lets the attacker monetize access to the victim’s resources.
